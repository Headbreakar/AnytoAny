...
interface PngToWebpClientProps {
  tool: ToolMeta;
}

export default function PngToWebpClient({ tool }: PngToWebpClientProps) {
  ...
  const handleFilesAccepted = async (files: File[]) => {
    ...
    try {
      ...
      const result = await convertImageFormat(selectedFile, "image/webp");
      clearInterval(progressInterval);
      setProgress(100);
      ...
      setTimeout(() => {
        ...
        setConvertedName(`${nameWithoutExt || "converted"}.webp`);
      }, 200);
    } catch (err: unknown) {
      setConverting(false);
      setError({
    ...
    <div className="space-y-6">
      {!file && !converting && (
        <DropZone
          ...
          label="drag & drop your PNG image"
        />
      )}
        ...
        <div className="space-y-6 animate-fade-in">
          ...
          {converting && (
            <div className="p-6 rounded-xl bg-slate-900/30 border border-stone-900/15">
              <ProgressBar progress={progress} label="Converting PNG to WebP..." />
            </div>
          )}
