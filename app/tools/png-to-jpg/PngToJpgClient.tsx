...
interface PngToJpgClientProps {
  tool: ToolMeta;
}

export default function PngToJpgClient({ tool }: PngToJpgClientProps) {
  ...
  const handleFilesAccepted = async (files: File[]) => {
    ...
    try {
      ...
      const result = await convertImageFormat(selectedFile, "image/jpeg");
      clearInterval(progressInterval);
      setProgress(100);
      ...
      setTimeout(() => {
        ...
        setConvertedName(`${nameWithoutExt || "converted"}.jpg`);
      }, 200);
    } catch (err: unknown) {
      setConverting(false);
      setError({
        ...
        <div className="space-y-6 animate-fade-in">
          ...
          {converting && (
            <div className="p-6 rounded-xl bg-slate-900/30 border border-white/5">
              <ProgressBar progress={progress} label="Converting PNG to JPG..." />
            </div>
          )}
