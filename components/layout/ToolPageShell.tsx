...
export default function ToolPageShell({
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-10 sm:px-6 lg:px-8 sm:py-14">
      ...
      <Link
        href="/"
        className="opacity-0-start animate-fade-in inline-flex items-center gap-2 text-sm font-medium text-stone-600 hover:text-stone-900 transition-colors group mb-8"
      >
        <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-stone-900/5 border border-stone-900/15 group-hover:border-primary/30 group-hover:bg-primary/10 transition-all duration-300">
      ...
      <div className="space-y-10">
        <header className="opacity-0-start animate-fade-in-up delay-100 text-center sm:text-left space-y-4">
          <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2">
            ...
            <span className="inline-flex items-center gap-1 rounded-full bg-stone-900/5 border border-stone-900/15 px-3 py-1 text-xs font-medium text-stone-600">
              <Sparkles className="h-3 w-3 text-secondary" />
              {categoryLabel}
            </span>
          </div>
          <h1 className="font-display text-3xl sm:text-4xl lg:text-5xl font-black tracking-tight text-stone-900 leading-tight">
            {title}
          </h1>
        ...
        <section className="opacity-0-start animate-fade-in-up delay-300 section-glow pt-10 space-y-4">
          <h2 className="font-display text-xl font-bold text-stone-900">{howToTitle}</h2>
          <ol className="space-y-3">
            {tool.howTo.map((step, idx) => (
              <li
                key={idx}
                className="flex gap-4 rounded-xl border border-stone-900/15 bg-bg-card/30 p-4 text-sm text-stone-600 transition-colors hover:border-stone-900/15 hover:bg-bg-card/50"
              >
        ...
        <section className="opacity-0-start animate-fade-in-up delay-400 space-y-5">
          <h2 className="font-display text-xl font-bold text-stone-900 text-center sm:text-left">
