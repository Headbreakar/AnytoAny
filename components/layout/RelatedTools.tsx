...
export default function RelatedTools({ currentSlug, category }: RelatedToolsProps) {
  ...
  return (
    <section className="border-t border-white/5 pt-12">
      <h2 className="font-display text-2xl font-bold text-white mb-6">Related tools</h2>
        ...
        {related.map((tool) => (
          ...
          >
            <div className="flex items-center gap-4">
              <span className="flex h-11 w-11 items-center justify-center rounded-xl bg-white/5 border border-white/10 group-hover:border-primary/30 group-hover:bg-primary/10 transition-all duration-300">
                <ToolIcon
                  name={tool.icon}
              ...
              <div className="min-w-0 flex-1">
                <h3 className="font-display font-semibold text-white text-sm group-hover:text-primary transition-colors truncate">
                  {tool.name}
                </h3>
                <p className="mt-0.5 text-xs text-slate-500 line-clamp-1">{tool.description}</p>
              </div>
